...
$exports = @(Get-ArrayValue $template "exports")
$messageTypes = @(Get-ArrayValue $template "messageTypes")
...
$entry = [pscustomobject][ordered]@{
    ...
    publication = [pscustomobject][ordered]@{
        ...
        packageSources = @(
            ...
            [pscustomobject][ordered]@{ kind = "rawFallback"; url = $rawFallbackUrl; assetName = $assetName; sha256 = $hash; sizeBytes = $size; releaseTag = $ReleaseTag; priority = 1 },
            [pscustomobject][ordered]@{ kind = "workspaceLocal"; url = $workspaceLocalUrl; assetName = $assetName; sha256 = $hash; sizeBytes = $size; releaseTag = $ReleaseTag; priority = 2 }
        )
    }
